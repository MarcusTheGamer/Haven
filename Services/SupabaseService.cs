...
public class SupabaseService : IAuthService
{
    ...
    public async Task<bool> LoginAsync(LoginModel login)
    {
        try
        {
            var session = await _client.Auth.SignIn(login.Email, login.Password);
            return session != null;
        }
        catch
        {
    ...
    public async Task<bool> SignUpAsync(SignUpModel signUp)
    {
        try
        {
            var session = await _client.Auth.SignUp(signUp.Email, signUp.Password, new SignUpOptions
            {
    ...
    public async Task LogoutAsync() => await _client.Auth.SignOut();

    public Task<UserProfile?> GetCurrentUserAsync()
    {
        var user = _client.Auth.CurrentUser;
        if (user == null) return Task.FromResult<UserProfile?>(null);

        var firstName = string.Empty;

        if (user.UserMetadata != null && user.UserMetadata.TryGetValue("first_name", out var value) && value != null)
        {
